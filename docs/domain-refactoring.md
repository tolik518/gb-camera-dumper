...
Folds in `android-refactoring.md` §7, reframed as context-map work:

- **Move RGB Merge into `gbcam-core`** (§7.1) — realign code with the context;
  consume `Photo.pixels_indexed` directly, drop the mono-PNG re-decode round-trip.
  Keep `RgbMergeDetector.java` until core output matches on real saves.
- **Make the on-disk PNG palette-independent** (§7.2) — keep instant recolor from
  `indexedPixels`; the baked PNG becomes a pure fallback cache.
...
**Not phases (opportunistic):** the Tier 2 value objects (§3) and the read-model
rename (`GalleryState` → `GalleryView`, or just a class-doc note saying it *is* a
read model). Land these whenever you're already editing the file; never as a
standalone churn commit.

Suggested commit order: A → B → C → D → (E) → F.
